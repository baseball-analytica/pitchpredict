...
async def build_dataset_vectorized(
    ...
    """
    import time
    from pitchpredict.backend.algs.deep.split import split_saved_dataset
    ...
    if split_val_ratio > 0 or split_test_ratio > 0:
        logger.info("Splitting dataset (val=%.1f%%, test=%.1f%%)...",
                    split_val_ratio * 100, split_test_ratio * 100)
    ...
    logger.info(
        ...
        total_time, stats.total_tokens, stats.total_pitches, stats.session_count, stats.pa_count
    )
